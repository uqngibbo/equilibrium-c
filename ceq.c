...
void solve_matrix(double* A, double* B, double *X, int N){
    /* Bare bones Gaussian matrix solution A*X = B where A is NxN and B and X are N elements
...
void compute_G0_RT(double T, double* lewis, int isp){
}

void pt(double p, double T, double* X0, int nsp, int nel, double* lewis, int nlewis, double* M, double* X1){
    return;
}

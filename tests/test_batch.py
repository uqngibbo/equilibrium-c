...
class TestCEQ(unittest.TestCase):
    def test_pt(self):
        ...
        for i in range(np):
            ...
            Ti = T[i]
            pi = p[i]
            Xs1i = eq.pt(pi, Ti, Xs0i)
            Xst[i,:] = Xs1i[:]

        Xs1 = eq.batch_pt(p, T, Xs0, 0)
    ...
    def test_rhou(self):
        ...
        for i in range(np):
            ...
            ui = u[i]
            rhoi = rho[i]
            Xs1i, Ti = eq.rhou(rhoi, ui, Xs0i)
            Xst[i,:] = Xs1i[:]
            Tt[i] = Ti

        Xs1, T = eq.batch_rhou(rho, u, Xs0, 0)
        assert_array_almost_equal(Xs1, Xst)
    ...
    #        pi = p[i]
    #        si = s[i]
    #        Xs1i, Ti = eq.ps(pi, si, Xs0i)
    #        Xst[i,:] = Xs1i[:]
    #        Tt[i] = Ti

    #    Xs1, T = eq.batch_ps(p, s, Xs0, 0)
    #    assert_array_almost_equal(Xs1, Xst)
    #    assert_array_almost_equal(T, Tt)
    #    return
